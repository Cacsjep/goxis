...
func parseCredentials(credentialsString string) (*shared.VapixCreds, error) {
	parts := strings.SplitN(credentialsString, ":", 2)
	if len(parts) != 2 {
		return nil, fmt.Errorf("error parsing credential string '%s'", credentialsString)
	}
	return &shared.VapixCreds{Username: parts[0], Password: parts[1]}, nil
}

func RetrieveVapixCredentials(username string) (*shared.VapixCreds, error) {
